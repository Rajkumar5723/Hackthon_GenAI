...
@clerk_require_auth
def imagekit_auth():
    # Placeholder for ImageKit authentication (implement actual logic)
    return jsonify({
        "token": "dummy_token",
        "expire": 1234567890,
...
# Create new chat with optional file upload
...
def create_chat():
    ...
    if 'file' in request.files:
        ...
        if file and allowed_file(file.filename):
            filename = secure_filename(f"{datetime.utcnow().isoformat()}-{uuid.uuid4()}{os.path.splitext(file.filename)[-1]}")
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            file.save(file_path)

    new_chat = {
        "userId": user_id,
        "history": [{"role": "user", "parts": [{"text": text, "file": file_path}]}]
    }
    ...
    return jsonify({"chatId": str(inserted_chat.inserted_id), "file": file_path}), 201
...
def update_chat(chat_id):
    ...
    new_items = []
    if question:
        user_part = {"role": "user", "parts": [{"text": question, "img": img}]}
        new_items.append(user_part)
    if answer:
        model_part = {"role": "model", "parts": [{"text": answer}]}
        new_items.append(model_part)
